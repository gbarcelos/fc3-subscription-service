...
class DefaultCancelSubscriptionTest extends UnitTest {
  ...
  public void givenActiveSubscription_whenCallsCancelSubscription_shouldCancelIt() {
    ...
    var expectedSubscription = newSubscriptionWith(expectedAccount.id(), expectedPlan,
        ActiveSubscriptionStatus.ACTIVE, LocalDateTime.now().minusDays(15));
    ...
    // when
    var actualOutput =
        this.target.execute(new CancelSubscriptionTestInput(expectedSubscriptionId.value(),
  ...
  record CancelSubscriptionTestInput(String subscriptionId, String accountId) implements
