...
class DefaultRemoveFromGroupTest extends UnitTest {
  ...
  public void givenCanceledSubscriptionAndDueDatePast_whenCallsExecute_shouldCallIdentityProvider() {
    ...
    final var johnsSubscription = newSubscriptionWith(expectedAccountId, plus,
        CanceledSubscriptionStatus.CANCELED, LocalDateTime.now().minusDays(1));
    final var expectedSubscriptionId = johnsSubscription.id();

    Assertions.assertTrue(johnsSubscription.isCanceled(),
        "Para esse teste a subscription precisa estar com Canceled status");
    ...
    // when
    this.target.execute(
        new RemoveFromGroupTestInput(expectedAccountId.value(), expectedGroupId.value(),
    ...
    verify(identityProviderGateway, times(1)).removeUserFromGroup(eq(john.userId()),
        eq(expectedGroupId));
  }
