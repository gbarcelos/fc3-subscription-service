  ...
  public ChangePlan.Output execute(final ChangePlan.Input in) {
    final var aPlan = this.planGateway.planOfId(new PlanId(in.planId()))
        .orElseThrow(() -> DomainException.with(
            "Plan with id %s could not be found".formatted(in.planId())));

    aPlan.execute(new PlanCommand.ChangePlan(in.name(), in.description(),
        new Money(in.currency(), in.price()), in.active()));
