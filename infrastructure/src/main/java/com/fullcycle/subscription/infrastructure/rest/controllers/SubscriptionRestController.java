...
/**
 * 1. Tornar o ID do IDP (Keycloak) o mesmo ID do usuário interno da aplicação (Account ID) 2.
 * Descobrir o Account ID com base no Subject do JWT 3. Colocar o Account ID dentro dos claims do
 * JWT
 */
@RestController
public class SubscriptionRestController implements SubscriptionRestApi {
  ...
  public SubscriptionRestController(
      final CreateSubscription createSubscription, final CancelSubscription cancelSubscription,
  ...
  public ResponseEntity<CreateSubscriptionResponse> createSubscription(
      final CreateSubscriptionRequest req, final CodeflixUser principal) {
    record CreateSubscriptionInput(Long planId, String accountId) implements
        CreateSubscription.Input {

    }
    final var res = this.createSubscription.execute(
        new CreateSubscriptionInput(req.planId(), principal.accountId()),
  ...
  public ResponseEntity<CancelSubscriptionResponse> cancelSubscription(
    ...
    final var res = this.cancelSubscription.execute(
        new CancelSubscriptionInput(principal.accountId()), CancelSubscriptionResponse::new);
    return ResponseEntity.ok(res);
  }

  @Override
  public ResponseEntity<ChargeSubscriptionResponse> chargeActiveSubscription(
      ChargeSubscriptionRequest req, CodeflixUser principal) {
    ...
    final var res = this.chargeSubscription.execute(
        new ChargeSubscriptionInput(principal.accountId(), req.paymentType(),
            req.creditCardToken()), ChargeSubscriptionResponse::new);
    return ResponseEntity.ok(res);
  }
}
